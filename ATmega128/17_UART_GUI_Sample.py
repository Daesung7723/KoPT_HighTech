...
class UartApp:
    ...
    # ## 수정: process_queue 함수에 새로운 ADC/ANGLE 데이터 처리 로직 적용
        ...
        for _ in range(100):
            ...
            try:
                ...
                try:
                    ...
                    # 새로운 데이터 포맷 확인
                    adc_match = re.search(r'\[ADC\](\d+)', decoded_data)
                    angle_match = re.search(r'\[ANGLE\](-?\d+)', decoded_data)
                    lcd0_match = re.search(r'\[LCD Data-0\](.*)', decoded_data)
                    lcd1_match = re.search(r'\[LCD Data-1\](.*)', decoded_data)

                    if adc_match:
                        self.adc_val = adc_match.group(1)
                        self.adc_value_label.config(text=self.adc_val)
                        continue
                    
                    elif angle_match:
                        self.angle_val = int(angle_match.group(1))
                        self.update_gauge(self.angle_val)
                        continue
    ...
    def update_gauge(self, deg):
        if self.needle: self.gauge_canvas.delete(self.needle)
    ...
    def send_data(self):
        ...
        if data_to_send:
            ...
            except serial.SerialException as e: self.update_status(f"전송 오류: {e}", "red")
